...
def Gen_Samples(X, y, n_to_sample, cl):
    ...
    samples = image_base + np.multiply(np.random.rand(n_to_sample, 1), image_neighbors - image_base)

    # use 10 as label because 0 to 9 real classes and 1 fake/smoted = 10
    return samples, [cl] * n_to_sample


def main(training_img_dir: Path = (".../MNIST/trn_img/"), training_label_dir: Path = Path(".../MNIST/trn_lab/")
    ...
    for i in range(len(label_training_files)):
        ...
        training_image_file = image_training_files[i]
        training_label_file = label_training_files[i]

        log.info(f"Training on image file: {training_image_file}")
        log.info(f"Training on label file: {training_label_file}")

        decoded_images = np.loadtxt(training_image_file)
        decoded_labels = np.loadtxt(training_label_file)
        ...
        log.info("Training images after reshape: {decoded_images.shape}")
        ...
        t0 = time.time()
        log.info(f"Fold {i} started at {t0}")
        if args["train"]:
            ...
            for epoch in range(args["epochs"]):
                ...
                for images, labs in train_loader:
                    ...
                    images_in_current_training_class = decoded_images[decoded_labels == current_training_class]
                    labels_in_current_training_class = decoded_labels[decoded_labels == current_training_class]
                    ...
                    indices_of_samples = np.random.choice(list(range(len(images_in_current_training_class))), number_of_samples, replace=False)
                ...
                if train_loss < best_loss:
                    log.info(f"Epoch({epoch}) training loss = {train_loss}\tBest training loss = {best_loss}")
            ...
            path_enc = model_output_path / f"fold_{i}" / "/final_encoder.pth"
            path_dec = model_output_path / f"fold_{i}" / "/final_decoder.pth"
            print(path_enc)
            print(path_dec)
            torch.save(encoder.state_dict(), path_enc)
            torch.save(decoder.state_dict(), path_dec)
        ...
        t1 = time.time()
        log.info(f"Fold {i} took: {(t1 - t0) / 60:.2f} minutes")
